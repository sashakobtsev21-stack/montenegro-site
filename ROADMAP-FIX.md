...
- [x] 🟡 **B2 — перенос длинных URL-кредитов фото** (анти-overflow на мобайле) — **сделано (2026-06-30)**. Портировано из эталона albania-site `9f0d46f`. Длинный неразрывный URL источника в кредите фото (`sourceUrl: https://commons.wikimedia.org/wiki/File:…`, особенно URL-кодированная кириллица в cover-кредите статьи `best-beaches-in-montenegro`) тянул страницу за вьюпорт на мобайле — одно «слово» без пробелов давало горизонтальный перелив (замер: +1175px @320px, и даже +279px @1280px). Фикс — чисто CSS в `src/styles/global.css`: `overflow-wrap: anywhere; word-break: break-word;` на кредитах/подписях `.prose figcaption, .prose .figure__credit, .cover__credit` + кредиты компонентов этого репо (`.gallery__credit, .rcard__credit, .rt__stop-credit`). В `scripts/qa-responsive.mjs` в PAGES добавлена статья с длинным URL-кредитом (`/attractions/best-beaches-in-montenegro/`), чтобы фикс реально проверялся. `qa:responsive`=GO (0 переполнений, 10 шаблонов × 5 ширин 320/360/414/768/1280), qa=GO, test:links=GO. Паритет en↔ru не затронут (правка только CSS).

## Доступность (a11y) — продолжение
- [x] 🟡 **B3 — тап-зона стрелок витрины-карусели ≥44×44px** (мобайл, WCAG 2.5.5) — **сделано (2026-06-30)**. Общий движок (эталон gruzia `ShowcaseRail.astro`). Стрелки `.scard__nav` визуально были пилюлей 30×30px → кликабельная `<button>` меньше рекомендованного тач-таргета 44px. Фикс — чисто CSS в `src/components/ShowcaseRail.astro`: сама кнопка теперь `min-width:44px; min-height:44px` с прозрачным фоном и контентом по центру (`display:flex; align-items:center; justify-content:center`), а видимая пилюля ~30px перенесена в `::before` (центрирована в 44px-зоне). Контраст-фикс a11y сохранён 1:1 — пилюля по-прежнему slate 60% + белая обводка 40% (hover 78%, WCAG 1.4.11); иконка (`svg`) поднята `z-index:1` над пилюлей; позиционирование (`left/right: var(--space-2)`, `top:50%`) и появление по hover/focus не изменены, стрелки карточки не перекрывают. `qa`=GO, `qa:responsive`=GO (0 переполнений), `test:links`=GO. Паритет en↔ru не затронут (правка только CSS).
