> ⚠️ **СПРАВОЧНИК, НЕ АКТИВНЫЙ ПЛАН (суперседед 2026-06-22).** Активный по-дневной контент-план — только в `/KALENDAR.md` (построен по SEO-аудиту, живой). Этот файл — **тематический бэклог кластеров и справка**: целевые EN-запросы, денежные связки/партнёры по разделам. Статус-колонки убраны намеренно (они врали по факту репо) — что опубликовано, смотреть в `KALENDAR.md` и `src/content/`. **Не редактировать как план и не сверять статусы по нему.**

# CONTENT_PLAN.md — справочник кластеров (Montenegro Guidebook)

Реюзабельная справка: какие тематические кластеры ведём, целевые EN-запросы и денежные связки по разделам.
Источник правил написания — `CONTENT_GUIDE.md` (как добавлять статью) и `SPEC.md` → `CLAUDE.md` → `docs/STRATEGY.md` (продукт/ниша/стратегия). Детальный кластерный план с целевыми EN-запросами и денежными связками — `docs/MONTENEGRO-ANALYSIS-2026-06-20.md` §5.

- **Языки (EN-first):** каждая тема = **en (ведущий)** + ru, одинаковый slug. URL: en на корне `/<category>/<slug>/`, ru → `/ru/…`. EN пишется первым и задаёт целевой запрос. (uk удалён 2026-06-22; старые `/uk/`-URL → 301 на корень.)

---

## 0. Стратегическое направление (из `docs/STRATEGY.md`)

- **EN-first / Tier-1 первично** (западные туристы и номады; дорогой RPM), CIS (ru) — крепкий вторичный (uk удалён 2026-06-22). Приоритет и подбор тем — от **EN-интента**.
...
> Целевые EN-запросы ниже — гипотезы спроса; пересортировывать по реальному спросу GSC. Денежные связки — `SPEC.md` §16 (сегментация /go/ по языку: EN → SafetyWing/DiscoverCars/GetYourGuide/Booking; CIS → EKTA/Localrent/Aviasales).
...
| Тема | Целевой EN-запрос | Тип | Ссылается вниз на |
|---|---|---|---|
...
| Тема | Целевой EN-запрос | Slug (`/car-rental/…`) |
|---|---|---|
...
| Тема | Целевой EN-запрос | Slug (`/insurance/…`) |
|---|---|---|
...
| Тема | Целевой EN-запрос | Slug (`/routes/…`) |
|---|---|---|
...
| Тема | Целевой EN-запрос | razvlType |
|---|---|---|
...
## 11. Предусловия к контенту (справка)

- **Партнёрки** — в `partners.json`: DiscoverCars, GetYourGuide/Viator, Booking (опц. Wise/Saily); починить EKTA-атрибуцию (`SPEC.md` §16, открытые пункты — `ROADMAP-FIX.md`).
